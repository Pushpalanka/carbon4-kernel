        ...
        try {
            ...
            for (CacheImpl cache : caches) {
                try {
                    ...
                    if (log.isDebugEnabled()) {
                        log.debug("Error occurred while running CacheCleanupTask for cache " + cache.getName(), e);
                    }
                } catch (Throwable e) {
                    log.error("Error occurred while running CacheCleanupTask for cache " + cache.getName(), e);
                }
            }
        } finally {
